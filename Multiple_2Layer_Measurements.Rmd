---
title: "Multiple layer measurements 2 LAYERS"
author: "Joan Ginard"
...
In this R markdown file, we will execute necessary steps to apply multi-layer graph measures to our data, **considering only FA and fMRI layers**.
It is exactly equal to R markdown with three layers to facilitate comparison
...
for(i in seq_along(FA_matrices)){
  NodeTensor<- list(FA_svd[[i]], fMRI_svd[[i]]) #Make list element combining on matrx from each layer
  NodeTensor_list[[i]] <- NodeTensor  # add element to a list
  
  all_values <- c(FA_svd[[i]], fMRI_svd[[i]])
...
p <- ggplot(x_df, aes(x = values, fill = controls_ms)) +
  ...
  labs(x = "Values of weight connections", title = "Weights distribution in 'average interlink' 2 layers")

ggsave(filename = "images/Mean_couplings_hist_2layers.png", plot = p, width = 10, height = 7, dpi = 300)
p


```

We observe that values are clustered between 0.145 and 0.155 (with one notable exemption). This suggests that differences are minimal (even when considering both HS and PwMS), and we can apply same weight to all subjects.
...
As it seems all coupling lead to fair same results, though details might be different, we use the last result and save it to csv, using all 50 that are statistically significant to differentiate between HS and PwMS.

Comparing to three layers, we have more statistical significant features, but only 2 passes bonferroni test, compared to 4 in three layers.
...
corrplot::corrplot(correlations, tl.col = "black",  
...
*save image*
```{r}
png("images/corr_2_layers_local.png")
corrplot::corrplot(correlations, tl.col = "black",  tl.cex = 0.6)
dev.off()
```
...
*save image*
```{r}
png("images/corr_2_layers_local_bonf.png")
corrplot::corrplot(correlations, method = 'number', tl.col = "black")
dev.off()
```
...
write.table(multi_layer_df, file = file.path(dataDIR, "multi_2layer.csv"), sep = ",", quote = FALSE, row.names = FALSE )
```
...
openxlsx::saveWorkbook(wb, file = paste0(dataDIR, "summary_multi_2layer.xlsx"), overwrite = TRUE)
