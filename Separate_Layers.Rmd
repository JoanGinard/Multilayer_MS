---
title: "Separate layer measurements"
author: "Joan Ginard"
...
In fact, we will create two equal R markdown files.  This one operates without normalizing the matrices, while the other involves normalization.
...
#### CREATE LABELS for clinical data
...
### Load graphs without svd normalization
```{r}
FA_graphs <- load_graphs(FADIR, clinical_data, normalization = FALSE)
GM_graphs <- load_graphs(GMDIR, clinical_data, normalization = FALSE)
fMRI_graphs <- load_graphs(fMRIDIR, clinical_data, normalization = FALSE)
...
#Iterate over measurements and get mean and sd for HS and PWMS
for(i in 2:ncol(df_temp)){
  measurement <- colnames(df_temp)[i] #Get measuremnt name
...
ggsave(filename = "images/FA_sep_layers_boxplot.png", plot = p, width = 10, height = 7, dpi = 300)
p

```
We can observe that some measures are fairly similar for both HS and PWMS. Additionally, it may be the case that certain measures might be strongly correlated.

As we are going to discard non statistically significant measurements, we save for later to examine correlations.
...
ggsave(filename = "images/GM_sep_layers_boxplot.png", plot = p, width = 10, height = 7, dpi = 300)
p


```
In this case, we observe that four measures have basically same value for HS and for PWMS and they seem to have constant values, while other three also seem quite similar.
...
ggsave(filename = "images/fMRI_sep_layers_boxplot.png", plot = p, width = 10, height = 7, dpi = 300)
p

```
This results are more similar to FA graphs. In this case we observe some very similar values, while others show noticeable differences.
...
print(stat_cols)
```
We can appreciate we do not have any measurement from GM layer
...
```
We can appreciate how there are high correlations, positive or negative, between most measurements that correspond to the same layer.

*save image*
```{r}
png("images/corr_sep_layers_global.png")
corrplot::corrplot(correlations, method = 'number', tl.col = "black")
dev.off()
```
...
```
It seems that global measures have difficulties differentiating between the mstypes. it is worth noting that we have very few instances of
SPMS and PPMS
...
stat_cols_loc <- df_summary_loc$measurement[df_summary_loc$p.value < 0.05 & !(is.na(df_summary_loc$p.value))]
...
```{r}
recap <- do.call(rbind, strsplit(stat_cols_loc_2, "_"))

recap <- data.frame(recap)
colnames(recap) <- c("LAYER", "Measure", "Node")
...
In final df we are going to include global measures which presented statistically significant between HS and PwMS and 90 local measures which passed bonferroni. 

As we have seen some of them are highly correlated but we will deal with it in a later script
...
```{r}
write.table(single_layer_df, file = file.path(dataDIR, "separate_layers.csv"), sep = ",", quote = FALSE, row.names = FALSE )
```
...
openxlsx::saveWorkbook(wb, file = paste0(dataDIR, "summary_sep_layers.xlsx"), overwrite = TRUE)
