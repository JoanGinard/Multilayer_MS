...
```
As in normalized case, only one measure passes this test. Although is a different measure and quite interestingly is the only one we had from the FA layer
...
```
We have alse a fewer measurement that pass the test than in normalized case.
...
```
In non-normalized case we had almost 100 local graph measures that passed both statistical tests, but in this case we have a very limited number and almost all of the measures come from closeness centrality. Regarding the layers we have 4 measurements from FA layer and 3 from fMRI
